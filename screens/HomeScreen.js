...
export default function MainScreen() {
  ...
  const addMusic = async () => {
    ...
    return
    try {
      await MusicsModel.getMusicTest()
    } catch (error) {
      console.error(error)
    }
    return
    ...
    try {
      data.forEach( async (music, index) => {
        ...
        await MusicsModel.addMusic(music, (isDone) => {
          if (isDone)
            console.log((index+1) + " added  : " + music.name)
          else
            console.log((index+1) + " failed : " + music.name)
        })
      });
  ...
  const renderSuggestedArtists = ({ item }) => (
    <View>
      <TouchableOpacity 
        ...
        onPress={() => navigation.navigate("PlaylistScreen", {from: "HomeScreen", type: "artist", id: item.id})}
      >
        <Image
