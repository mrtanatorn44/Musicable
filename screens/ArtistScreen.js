...
export default function ArtistScreen ({route}) {
  const navigation = useNavigation();
  const artistInfo = "Popular Release" // get this from route params
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <View style={styles.artistImageContainer}>
          <Image source={{uri: artistData.image}} style={styles.artistImage}/>
          ...
          <View style={{fontSize: 50, position: 'absolute', bottom: 0, margin: '2%'}}>
            <Text style={{fontSize: 50, fontWeight: 'bold',color: 'white'}} numberOfLines={1}>{artistData.name}</Text>
            <Text style={{fontSize: 18, color: 'lightgray'}}><Octicons name="heart-fill" size={18} color="lightgray" /> {getNumber(artistData.followed)} follower</Text>
          </View>
        </View>

        {/* Song list */}
        <View style={{height: '5%', justifyContent: 'center'}}>
          <Text style={{fontSize: 24, fontWeight: 'bold', color: 'white', letterSpacing: 1}} numberOfLines={1}>{"  " + artistInfo}</Text>
        </View>
